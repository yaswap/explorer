      ...
      if (orders['bids'].length > 0) {
        for (var i = 0; i < orders['bids'].length; i++) {
          var order = {
            ...
            total: (parseFloat(orders.bids[i].remaining_volume).toFixed(8) * parseFloat(orders.bids[i].price)).toFixed(
              8
            ),
          };
          buys.push(order);
      ...
      if (orders['asks'].length > 0) {
        for (var x = 0; x < orders['asks'].length; x++) {
          var order = {
            ...
            // Necessary because API will return 0.00 for small volume transactions
            total: (parseFloat(orders.asks[x].remaining_volume).toFixed(8) * parseFloat(orders.asks[x].price)).toFixed(
              8
            ),
          };
          sells.push(order);
