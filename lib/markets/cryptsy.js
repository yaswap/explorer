      ...
      if (orders['buyorders'].length > 0) {
        for (var i = 0; i < orders['buyorders'].length; i++) {
          var order = {
            ...
            total: (
              parseFloat(orders.buyorders[i].quantity).toFixed(8) * parseFloat(orders.buyorders[i].price)
            ).toFixed(8),
          };
          buys.push(order);
        }
      ...
      if (orders['sellorders'].length > 0) {
        for (var x = 0; x < orders['sellorders'].length; x++) {
          var order = {
            ...
            // Necessary because API will return 0.00 for small volume transactions
            total: (
              parseFloat(orders.sellorders[x].quantity).toFixed(8) * parseFloat(orders.sellorders[x].price)
            ).toFixed(8),
          };
          sells.push(order);
        }
