...
async function update_address(hash, blockheight, tx, amount, type, tx_info, cb) {
  ...
  */
  ...
  try {
    ...
    // If this UTXO isn't timelocked UTXO, return immediately
    if (!isTimeLockedUtxo) {
      return cb();
    }
...
module.exports = {
  ...
  get_timelockinfo: async function () {
    var timelockinfo = [];
    var timelocklist = await TimeLock.find({}).sort({ balance: 'desc' }).limit(100);

    for (var item in timelocklist) {
      var info = {
        address: item.a_id,
                        ...
                        function (txid, next_tx) {
                          Tx.findOne({ txid: txid }, function (err, tx) {
                            if (tx) {
                              ...
                              save_tx(txid, block, function (err) {
                                if (err) {
                                  console.log(err);
                                } else {
                                  console.log('%s: %s', block_height, txid);
