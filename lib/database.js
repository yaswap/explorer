...
function update_address(hash, blockheight, txid, amount, type, prevtxid, vout, cb) {
  var to_sent = false;
      ...
      lib.prepare_vin(tx, function(vin) {
        lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
          lib.syncLoop(vin.length, function (loop) {
            ...
            update_address(nvin[i].addresses, blockheight, txid, nvin[i].amount, 'vin', null, null, function(){
              loop.next();
            });
          }, function(){
            lib.syncLoop(vout.length, function (subloop) {
              ...
              if (vout[t].addresses) {
                ...
                update_address(vout[t].addresses, blockheight, txid, vout[t].amount, 'vout', null, t, function(){
                  subloop.next();
                });
              } else {
                subloop.next();
