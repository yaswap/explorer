...
async function update_address(hash, blockheight, txid, amount, type, tx_info, cb) {
  ...
  try {
    ...
    if (timelockaddress.iscltv) {
      // Cltv address
      const timelock = await TimeLock.findOneAndUpdate(
        { a_id: hash },
        {
          $set: {
      ...
      // Csv address
      if (type == 'vin') {
        // use existing UTXO, update document of CsvInfo db
        await CsvInfo.findOneAndUpdate(
          { a_id: hash, txid: tx_info.prevTxid },
          {
            $set: {
              isused: true,
            },
        ...
        const csvinfo = await CsvInfo.find({ a_id: hash, isused: false }).sort({ timetouse_number: 'asc' }).limit(10);
        ...
        // new UTXO, add new document to CsvInfo db
        save_csvinfo(
          hash,
          txid,
          amount,
          timelockaddress.locktime,
          timelockaddress.istimebased,
          blockheight,
          async function (err) {
            if (err) {
              return cb(err);
              ...
              try {
                const csvinfo = await CsvInfo.find({ a_id: hash, isused: false })
                  .sort({ timetouse_number: 'asc' })
                  .limit(10);
...
module.exports = {
  ...
  add_timelock: function (describeinfo, cb) {
    ...
    if (describeinfo.RedeemScriptFormat.includes('OP_CHECKLOCKTIMEVERIFY')) {
      ...
      if (newTimeLock.locktime < 500000000) {
        // Block-based lock
      ...
      if (newTimeLock.locktime & SEQUENCE_LOCKTIME_TYPE_FLAG) {
        // Time-based lock
    ...
    find_address(newTimeLock.a_id, function (address) {
      ...
      if (newTimeLock.iscltv) {
        ...
        AddressTx.find({ a_id: newTimeLock.a_id })
          .sort({ blockindex: 'asc' })
              ...
              lib.syncLoop(
                ...
                function (loop) {
                  ...
                  update_csvinfo(
                    newTimeLock.a_id,
                    address_tx[i].txid,
                    newTimeLock.locktime,
                    newTimeLock.istimebased,
                    function (err) {
                      loop.next();
                    }
                  );
                ...
                function () {
                  CsvInfo.find({ a_id: newTimeLock.a_id, isused: false })
                    .sort({ timetouse_number: 'asc' })
                    .limit(10)
                    .exec(function (err, csvinfo) {
