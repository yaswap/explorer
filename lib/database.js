              ...
              if (type == 'vin') { // use existing UTXO, update document of CsvInfo db
                ...
                save_csvinfo(hash, txid, amount, timelockaddress.locktime, timelockaddress.istimebased, blockheight, function(err) {
                  if (err) {
                    ...
                    CsvInfo.find({a_id: address, isUsed: false}).sort({timetouse_number: 'asc'}).limit(10).exec(function(err, csvinfo){
                      TimeLock.updateOne({a_id: address}, {
                        $set: {
                          balance: address.balance,
                          csvinfo: csvinfo
                        }
...
function update_csvinfo(address, txid, locktime, istimebased, cb){
  Tx.find({txid: txid}, function (err, tx) {
    if (err) {
      return cb(err);
    }
...
function save_csvinfo(address, txid, amount, locktime, istimebased, blockheight, cb){
  lib.get_block_by_number(blockheight-1, function (err, block) {
    var timetouse_number = 0;
    var timetouse_string = "";

    if (istimebased) {
      timetouse_number = locktime + block.time;
      const date = new Date(timetouse * 1000);
      timetouse_string += "locked until " + date.toLocaleString();
    } else {
      timetouse_number = locktime + blockheight;
      timetouse_string += "locked until block height " + timetouse_number;
    }
