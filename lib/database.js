  ...
  if ( hash == 'coinbase' ) { // it must by type = 'vin'
    ...
    if (type == 'vin') {
      ...
      AddressUtxo.findOneAndUpdate(
        ...
        console.log("TACA ===> update_utxo, used UTXO = ", tx_info);
        return cb();
      });
    } else {
      addr_inc.received = amount;
      ...
      AddressUtxo.findOne({txid: txid}, function(err, result) {
        if (!result) {
          ...
          newAddressUtxo.save(function(){
            console.log("TACA ===> update_utxo, save new UTXO = ", newAddressUtxo);
            return cb();
          })
        }
        else
  ...
  Tx.findOne({txid: txid}, function (err, tx) {
    ...
    lib.syncLoop(tx.vin.length, function (loop) {
      ...
      if (tx.vin[i].addresses == address) {
        CsvInfo.findOneAndUpdate({a_id: address, txid: tx.vin[i].prevTxid}, {
          $set: {
            isused: true
          }
        }, function (err, csvinfo) {
...
function reindex_tx(txid, cb) {
  ...
  lib.get_rawtransaction(txid, function(tx){
    // Refer tx format http://explorer.yacoin.org/api/getrawtransaction?txid=51e24109397d06376fd61e50d9d59b4164d6a41bf64fcf959bdd79077f61e2ca&decrypt=1
    if (tx != 'There was an error. Check your console.') {
      console.log("TACA ===> reindex_tx, tx = ", tx)
      lib.prepare_vin(tx, function(vin) { // prepare list of {vin_address, sent_amount}
        lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) { // prepare list of {vout_address, received_amount}
          console.log("TACA ===> reindex_tx, nvin = ", nvin)
          console.log("TACA ===> reindex_tx, vout = ", vout)
          lib.syncLoop(vin.length, function (loop) {
            // Update balance and transaction of addresses used as vin
            var i = loop.iteration();
            tx_info = {
            ...
            console.log("TACA ===> reindex_tx, vin tx_info = ", tx_info)
            update_utxo(nvin[i].addresses, txid, nvin[i].amount, 'vin', tx_info, function(){
              console.log("TACA ===> reindex_tx, update_utxo vin[%s]", i)
              loop.next();
            });
          }, function(){
            lib.syncLoop(vout.length, function (subloop) {
              ...
              if (vout[t].addresses) {
                ...
                console.log("TACA ===> reindex_tx, vout tx_info = ", tx_info)
                update_utxo(vout[t].addresses, txid, vout[t].amount, 'vout', tx_info, function(){
                  console.log("TACA ===> reindex_tx, update_utxo vout[%s]", t)
                  subloop.next();
                });
              } else {
                subloop.next();
