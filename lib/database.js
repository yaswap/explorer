        ...
        TimeLock.findOne({a_id: hash}, function (err, timelockaddress) {
            if (!err && timelockaddress && !timelockaddress.iscltv) { // Csv address
              if (type == 'vin') { // use existing UTXO, update document of CsvInfo db
                ...
                lib.get_block_by_number(blockheight, function (err, block) {
                  var timetouse_number = 0;
                  var timetouse_string = "";

                  if (timelockaddress.istimebased) {
                    timetouse_number = timelockaddress.locktime + block.time;
                    const date = new Date(timetouse * 1000);
                    timetouse_string += "locked until " + date.toLocaleString();
                  } else {
                    timetouse_number = timelockaddress.locktime + blockheight;
