...
function find_utxo(address, cb) {
  AddressUtxo.find({a_id: hash, isused: false}, function(err, utxo) {
    if(utxo) {
      return cb(utxo);
    } else {
      return cb();
...
// hash = address
function update_address(hash, blockheight, txid, amount, type, tx_info, cb) {
  var to_sent = false;
  ...
  if ( hash == 'coinbase' ) { // it must by type = 'vin'
    ...
    if (type == 'vin') {
      ...
      newAddressUtxo.save(function(err) {
        if (err) {
          return cb(err);
        } else {
