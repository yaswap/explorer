...
function update_address(hash, blockheight, txid, amount, type, prevtxid, cb) {
  var to_sent = false;
        ...
        AddressTx.findOneAndUpdate({a_id: hash, txid: txid}, {
          ...
          if (err) {
            ...
            TimeLock.findOne({a_id: hash}, function (err, timelockaddress) {
              console.log("TACA database.js, update_address ===> hash = %s, timelockaddress = %s", hash, util.inspect(timelockaddress, false, null, true));
                if (!err && timelockaddress && !timelockaddress.iscltv) { // Csv address
                  if (type == 'vin') { // use existing UTXO, update document of CsvInfo db
                    CsvInfo.findOneAndUpdate({a_id: hash, txid: prevtxid}, {
                      $set: {
                        isused: true
                      }
                      ...
                      if (err) {
                        ...
                        CsvInfo.find({a_id: hash, isused: false}).sort({timetouse_number: 'asc'}).limit(10).exec(function(err, csvinfo){
                          TimeLock.findOneAndUpdate({a_id: hash}, {
                            ...
                            console.log("TACA database.js, update_address ===> after update timelock = %s", util.inspect(timelock, false, null, true));
                            if (err) {
                              return cb(err);
                            } else {
                              return cb();
                    ...
                    save_csvinfo(hash, txid, amount, timelockaddress.locktime, timelockaddress.istimebased, blockheight, function(err) {
                      if (err) {
                        console.log("TACA database.js, update_address ===> call save_csvinfo for hash = %s with error = %s", hash, err);
                        return cb(err);
                      } else {
                        CsvInfo.find({a_id: hash, isused: false}).sort({timetouse_number: 'asc'}).limit(10).exec(function(err, csvinfo){
                          console.log("TACA database.js, update_address ===> hash = %s, csvinfo = %s", hash, util.inspect(csvinfo, false, null, true));
                          TimeLock.findOneAndUpdate({a_id: hash}, {
                            $set: {
                              balance: address.balance,
                              csvinfo: csvinfo
...
function update_csvinfo(address, txid, locktime, istimebased, cb){
  Tx.findOne({txid: txid}, function (err, tx) {
    ...
    console.log("TACA database.js ===> update_csvinfo, address = %s, txid = %s, tx = %s", address, txid, util.inspect(tx, false, null, true));
    lib.syncLoop(tx.vin.length, function (loop) {
      var i = loop.iteration();
      console.log("TACA database.js ===> update_csvinfo, tx.vin.length = %s, i = %s", tx.vin.length, i);
      ...
      lib.syncLoop(tx.vout.length, function (subloop) {
        ...
        console.log("TACA database.js ===> update_csvinfo, tx.vout.length = %s, t = %s", tx.vout.length, t);
...
function save_csvinfo(address, txid, amount, locktime, istimebased, blockheight, cb){
  lib.get_block_by_number(blockheight-1, function (block) {
    console.log("TACA database.js, save_csvinfo ===> blockheight = %s, block = %s", blockheight, util.inspect(block, false, null, true));
    ...
    console.log("TACA database.js, save_csvinfo ===> newCsvInfo = %s", util.inspect(newCsvInfo, false, null, true));
    newCsvInfo.save(function(err) {
      if (err) {
        console.log("TACA database.js, save_csvinfo ===> save newCsvInfo unsuccessfully");
        return cb(err);
      } else {
        console.log("TACA database.js, save_csvinfo ===> save newCsvInfo successfully");
        return cb();
      }
    });
  });
      ...
      lib.prepare_vin(tx, function(vin) {
        lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
          lib.syncLoop(vin.length, function (loop) {
            var i = loop.iteration();
            console.log("TACA database.js, save_tx ===> update vin[%s/%s], address = %s, amount = %s, nvin[i].txid = %s, nvin[i].vout = %s", i, vin.length, nvin[i].addresses, nvin[i].amount, nvin[i].txid, nvin[i].vout);
            update_address(nvin[i].addresses, blockheight, txid, nvin[i].amount, 'vin', nvin[i].txid, function(){
              loop.next();
            });
          }, function(){
            lib.syncLoop(vout.length, function (subloop) {
              ...
              if (vout[t].addresses) {
                ...
                update_address(vout[t].addresses, blockheight, txid, vout[t].amount, 'vout', null, function(){
                  subloop.next();
                });
              } else {
                subloop.next();
...
module.exports = {
  ...
  add_timelock: function(describeinfo, cb){
    ...
    find_address(newTimeLock.a_id, function(address) {
      ...
      console.log("TACA database.js ===> add_timelock, address = %s, newTimeLock.iscltv= %s", newTimeLock.a_id, newTimeLock.iscltv);
      if (newTimeLock.iscltv) {
        console.log("TACA database.js ===> add_timelock, newTimeLock.iscltv");
        // Save document
        ...
        console.log("TACA database.js ===> add_timelock, newTimeLock.isCsv");
        ...
        AddressTx.find({a_id: newTimeLock.a_id}).sort({blockindex: 'asc'}).exec(function(err, address_tx){
          if (err) {
            ...
            lib.syncLoop(address_tx.length, function (loop) {
              var i = loop.iteration();
              console.log("TACA database.js ===> add_timelock, address_tx.length = %s, i = %s",address_tx.length, i);
              ...
              CsvInfo.find({a_id: newTimeLock.a_id, isused: false}).sort({timetouse_number: 'asc'}).limit(10).exec(function(err, csvinfo){
                console.log("TACA database.js ===> add_timelock, csvinfo = %s, err = %s", util.inspect(csvinfo, false, null, true), err);
