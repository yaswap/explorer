...
async function parse_P2PKH_timelock_script(vout, block) {
  ...
  if (timelockType == ScriptPubKeyType.CLTV_P2PKH_timelock) {
    ...
    // CSV
    newTimeLockUtxoInfo.iscltv = false;
    if (newTimeLockUtxoInfo.locktime & lib.SEQUENCE_LOCKTIME_TYPE_FLAG) {
      // Time-based lock
      // Convert to real seconds
      newTimeLockUtxoInfo.locktime =
        (newTimeLockUtxoInfo.locktime & lib.SEQUENCE_LOCKTIME_MASK) << lib.SEQUENCE_LOCKTIME_GRANULARITY;
    ...
    module.exports.syncLoop(
      ...
      async function (loop) {
        ...
        if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') {
          ...
          if (
            vout[i].scriptPubKey.type === ScriptPubKeyType.CLTV_P2PKH_timelock ||
            vout[i].scriptPubKey.type === ScriptPubKeyType.CSV_P2PKH_timelock
          ) {
            if (vout[i].scriptPubKey.type == ScriptPubKeyType.CSV_P2PKH_timelock && block === null) {
              block = await module.exports.get_block_promise(blockhash);
            }
            timelockUtxoInfo = await parse_P2PKH_timelock_script(vout[i], block);
          } else {
            // Check if it is P2SH timelock address
            const timelockaddress = await TimeLock.find({ a_id: vout[i].scriptPubKey.addresses[0] });
            if (
              timelockaddress &&
              (timelockaddress.type !== ScriptPubKeyType.CLTV_P2PKH_timelock ||
                timelockaddress.type !== ScriptPubKeyType.CSV_P2PKH_timelock)
            ) {
              if (timelockaddress.type.includes('CSV_P2SH_timelock') && block === null) {
                block = await module.exports.get_block_promise(blockhash);
              }
