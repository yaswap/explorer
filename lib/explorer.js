...
module.exports = {
  ...
  send_rawtransaction: function(rawtransaction, cb) {
    console.log("TACA ===> send_rawtransaction, rawtransaction = %s", rawtransaction)
    rpcCommand([{method:'sendrawtransaction', parameters: [rawtransaction]}], function(response){
      console.log("TACA ===> send_rawtransaction, response = %s", response)
      return cb(response);
    });
  },
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    if (input.coinbase) {
      ...
      module.exports.get_rawtransaction(input.txid, function(prevTx){
        if (prevTx) {
          module.exports.syncLoop(prevTx.vout.length, function (loop) {
            var i = loop.iteration();
          ...
          module.exports.is_unique(arr_vin, addresses[0].hash, function(unique, index) {
            if (unique == true) {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
                arr_vin.push({addresses:addresses[0].hash, amount:amount_sat, prevTxid: tx.vin[i].txid, prevVout: tx.vin[i].vout}); // TODO: need to add vout
                loop.next();
              });
            } else {
              module.exports.convert_to_satoshi(parseFloat(addresses[0].amount), function(amount_sat){
